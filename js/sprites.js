...
(function () {
  ...
  const ARCH = {
    ...
    mimic: { w: 16, h: 16, draw(g, p, f) {
      const s = f % 2, o = s ? 0 : 2;     // 뚜껑이 들썩인다
      // 뚜껑 (위로 벌어짐)
      rect(g, 1, 0 + s, 14, 4, p[0]); rect(g, 1, 0 + s, 14, 1, shade(p[0], 0.25)); rect(g, 7, 2 + s, 2, 2, p[2]);
      // 입 안 + 이빨 + 혀
      rect(g, 1, 4 + s, 14, 3 + o, '#2a0a10');
      for (let x = 2; x < 14; x += 3) { px(g, x, 4 + s, '#f4ecd8'); px(g, x + 1, 6 + s + o, '#f4ecd8'); }
      rect(g, 6, 6 + s + o, 5, 2, '#e05a7a'); px(g, 10, 8 + s + o, '#e05a7a');
      px(g, 4, 5 + s, '#ffe040'); px(g, 11, 5 + s, '#ffe040');
      // 몸통 (상자)
      rect(g, 1, 8 + o, 14, 7 - o, p[1]); rect(g, 1, 8 + o, 14, 1, p[2]);
      rect(g, 1, 8 + o, 1, 7 - o, shade(p[1], -0.3)); rect(g, 7, 9 + o, 2, 3, p[2]);
      rect(g, s ? 2 : 3, 15, 3, 1, shade(p[1], -0.4)); rect(g, s ? 11 : 10, 15, 3, 1, shade(p[1], -0.4));
    } },
  };
  ...
  // 축복의 제단 받침돌 (구슬은 render 에서 빛나게 따로 그린다)
  S.shrine = function (used) {
    return get('shrine:' + (used ? 1 : 0), () => outlined(14, 16, (g) => {
      rect(g, 1, 12, 12, 4, '#6a6478'); rect(g, 1, 12, 12, 1, '#9a94a8');
      rect(g, 3, 5, 8, 7, '#8a8498'); rect(g, 3, 5, 8, 1, '#b8b2c8'); rect(g, 3, 5, 1, 7, '#6a6478');
      rect(g, 2, 3, 10, 2, '#a8a2b8'); rect(g, 2, 3, 10, 1, '#d8d2e8');
      rect(g, 5, 7, 4, 3, used ? '#4a4458' : '#3a3448'); px(g, 6, 8, used ? '#6a6478' : '#e8e0ff');
      px(g, 4, 13, '#4a4458'); px(g, 9, 14, '#4a4458');
    }));
  };
  ...
  const FONT = {
    ...
    A: '010101111101101', C: '011100100100011', E: '111100110100111', G: '011100101101011', I: '111010010010111', K: '101101110101101',
    L: '100100100100111', N: '101111111111101', R: '110101110101101', T: '111010010010010', U: '101101101101111', X: '101101010101101',
  };
